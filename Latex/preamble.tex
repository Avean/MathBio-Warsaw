...
% Slides are read at a glance, so no word may ever be broken across lines.
% \hyphenpenalty alone is not enough: in a narrow column TeX still hyphenates
% once no unhyphenated break fits within \tolerance.  Raising the tolerance and
% allowing emergency stretch lets it set a slightly loose line instead.
\hyphenpenalty=10000
...
% Section and local slide number, bottom right. Drawn as an overlay so it costs no text height;
% \source reserves the adjacent area to its left.
\setbeamertemplate{footline}{%
  \begin{tikzpicture}[remember picture,overlay]
    \node[anchor=south east,
      font=\fontsize{6.6}{7.6}\selectfont]
      at ([xshift=-0.70cm,yshift=0.20cm]current page.south east)
      {{\color{HydraGold}\bfseries\hydraTwoDigits{\value{section}}}%
       \hspace{0.16em}{\color{HydraMuted}/}\hspace{0.16em}%
       {\color{HydraWhite}\hydraTwoDigits{\numexpr\value{framenumber}-\hydraSectionFrameBase\relax}}};
  \end{tikzpicture}%
}
