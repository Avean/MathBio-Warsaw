...
\newcommand{\hydraSectionImage}{figures/common/hydra_zoom_map.png}
...
\newcommand{\hydraTocLine}[3]{%
  ...
  \begin{tikzpicture}[baseline=(title.base)]
    ...
    \node[anchor=west,text=#2,text width=5.05cm,align=left,
      font=#3\fontsize{8.9}{10.4}\selectfont]
      (title) at (0.62,0)
      {\hyphenpenalty=10000\exhyphenpenalty=10000\relax\inserttocsection};
  \end{tikzpicture}\par
    % \vspace{-0.50cm}%
}
...
\newcommand{\hydraSectionMapFrame}{%
  ...
  \begin{frame}[plain,noframenumbering]
    \begin{tikzpicture}[remember picture,overlay]
      ...
      \node[anchor=north west,text=HydraBody,text width=6.05cm,align=left,
...
\newcommand{\hydraSubsectionFrame}{%
  \begin{frame}[plain,noframenumbering]
    \begin{tikzpicture}[remember picture,overlay]
      ...
      % Clamp the pan so the zoomed background always covers the page.  A focus
      % point near an edge (section 6 sits at focus y=0.78) otherwise pushes the
      % image off the slide and leaves a bare navy strip along the bottom.
      \pgfmathsetlengthmacro{\hydraZoomCX}{%
        max(min(0.74*\paperwidth-\hydraZoom*(\hydraFocusX-0.5)*\paperwidth,
                0.5*\hydraZoom*\paperwidth),
            \paperwidth-0.5*\hydraZoom*\paperwidth)}
      \pgfmathsetlengthmacro{\hydraZoomCY}{%
        max(min(0.50*\paperheight-\hydraZoom*(0.5-\hydraFocusY)*\paperheight,
                0.5*\hydraZoom*\paperheight),
            \paperheight-0.5*\hydraZoom*\paperheight)}
      ...
      % .90 let bright parts of the photo ghost through the text panel as a
      % smudge behind the heading; .97 keeps the panel readable.
      \fill[HydraNavy,opacity=.97]
        (current page.south west)
...
\NewDocumentCommand{\HydraSection}{O{} m}{%
  ...
  \pgfkeys{/hydra section/.cd,
    ...
    radius=0.13,
    zoom=2.05,
    image=figures/common/hydra_zoom_map.png,
    #1}%
...
\NewDocumentCommand{\HydraSubsection}{O{} m}{%
  ...
  % Write the running total on every subsection, not only at \AtEndDocument:
  % the end-of-document hook fired too late to record the final section, so
  % section 6 had no entry in the .aux and its progress bars counted up from
  % one instead of showing all five.  The last write per section wins.
  \hydraWriteSubTotal
...
% -----------------------------------------------------------------------------
% A display environment (align*, \[...\]) leaves TeX in horizontal mode, so a
% \vspace written after one is turned into a \vadjust and lands *inside* the
% next paragraph instead of before it.  Without the \par below, that pulled the
% bullet text onto the same line as its marker and printed the dot on top of
% the first letter.  \par in vertical mode is a no-op, so this is always safe.
\NewDocumentCommand{\HydraPoint}{O{HydraGold} m m}{%
  ...
  \begin{minipage}[t]{\dimexpr\linewidth-0.40cm\relax}
    ...
    \vspace{-0.05cm}%
    {\color{HydraBody}\fontsize{6.8}{8.6}\selectfont #3\par}%
  \end{minipage}\par
  \vspace{0.32cm}%
}
...
\newcommand{\source}[1]{%
  \begin{tikzpicture}[remember picture,overlay]
    % No fixed text width: a short credit in a 13.6cm right-aligned box was
    % reported as an underfull hbox on every slide that used one.
    \node[anchor=south east,text=HydraMuted,font=\fontsize{6}{7.2}\selectfont,
      align=right,text width=13.6cm,inner sep=0pt]
      at ([xshift=-1.75cm,yshift=0.16cm]current page.south east)
      {\raggedleft\hspace{0pt}#1\par};
  \end{tikzpicture}%
}
...
\newcommand{\titlelayout}[1]{%
  \begin{tikzpicture}[remember picture,overlay]
    ...
    \node[anchor=north west,text=HydraWhite,text width=7.5cm,
      ...
      {\insertsubtitle};
    \node[anchor=south west,text=HydraWhite,text width=9.0cm,
      font=\bfseries\fontsize{10}{13}\selectfont]
      at ([xshift=0.76cm,yshift=1.52cm]current page.south west)
      {\insertauthor};
...
% -----------------------------------------------------------------------------
% Running gag.  Two slides: the question, then the answer.  Dropped in at the
% points where a model has just been shown to be insufficient.
%
% Both frames are [plain], so they carry no breadcrumb, no footline and no
% frame title, and [noframenumbering], so the gag does not advance the frame
% counter that the footline reads: the slide numbers of the section run on
% either side of it as if the two frames were not there.  The meme is exactly
% 16:9, the same as the paper, so it fills the slide edge to edge with nothing
% letterboxed.
% -----------------------------------------------------------------------------
\newcommand{\PerfectModel}{%
  \begin{frame}[plain,noframenumbering]
    \begin{tikzpicture}[remember picture,overlay]
      \fill[HydraNavy]
        (current page.south west) rectangle (current page.north east);
      \node[text=HydraWhite,font=\bfseries\fontsize{44}{52}\selectfont]
        at (current page.center) {Perfect model?};
    \end{tikzpicture}
  \end{frame}
  \begin{frame}[plain,noframenumbering]
    \begin{tikzpicture}[remember picture,overlay]
      ...
      \node[anchor=center,inner sep=0pt] at (current page.center)
        {\includegraphics[width=\paperwidth,height=\paperheight,keepaspectratio]
          {figures/common/well_yes_but_actually_no.jpg}};
    \end{tikzpicture}
  \end{frame}%
}
